...
public:
    SentimentAnalyzer(const std::string &lexiconFilePath, const std::string &stopWordsFilePath);
    bool isStopWord(const std::string &word) const;
    double analyze(const std::string &text) const;
    std::unordered_map<std::string, int> frequentWords(const std::vector<std::string> &texts, int topN) const;
};
